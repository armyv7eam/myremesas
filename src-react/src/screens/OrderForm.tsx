...
import { detectBankByPrefix } from '../lib/constants';
...
export function OrderForm({ isOpen, onClose, onSuccess }: Props) {
    ...
    // Cálculo en tiempo real del monto destino
        ...
        try {
            ...
            lines.forEach(line => {
                ...
                // Teléfono
                    ...
                    // Limpiar teléfono de espacios, guiones o signos +
                    const phoneClean = line.split(/[:\-]/)[1]?.replace(/\D/g, '') || '';
                    if (phoneClean.length >= 10) {
                        // if it has country code +58 (12 digits) or starts with 0 (11 digits)
    ...
    const handleSubmit = async () => {
        ...
        try {
            const orderId = await createOrder(data, rates);
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleClose} title="Crear Nuevo Pedido">
            <div className="space-y-4">
                ...
                {/* Montos: Recibir (Izquierda) - Enviar (Derecha) */}
