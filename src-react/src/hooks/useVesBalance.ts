...
export function useVesBalance() {
    ...
    const search = useCallback(async (startDate: Date, endDate: Date) => {
        ...
        try {
            ...
            const adds = entries.filter(e => e.type === 'add').reduce((s, e) => s + e.amount, 0);
            const subs = entries.filter(e => e.type === 'subtract').reduce((s, e) => s + e.amount, 0);
            const fees = entries.filter(e => e.type === 'fee').reduce((s, e) => s + e.amount, 0);
