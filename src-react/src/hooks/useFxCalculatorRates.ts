...
const fetchUsdtVesFromExternalRatesApi = async (apiBaseUrl: string): Promise<MarketQuote | null> => {
    try {
        const payload = await fetchJson<Record<string, any>>(apiBaseUrl);
        const usdtToVesDirect = Number(payload?.USDT_to_VES_P2P);
        const vesToUsdt = Number(payload?.VES_to_USDT_P2P);

        // Algunas fuentes etiquetan VES_to_USDT_P2P, pero publican realmente USDT->VES.
        // Heurística: si el valor es alto (ej: 600+), asumimos USDT->VES directo.
        // Si es pequeño (ej: 0.0016), lo invertimos para obtener USDT->VES.
        const usdtToVesFromLegacy = isPositive(vesToUsdt)
            ? (vesToUsdt > 20 ? vesToUsdt : (1 / vesToUsdt))
            : 0;

        const usdtToVes = isPositive(usdtToVesDirect)
            ? usdtToVesDirect
            : usdtToVesFromLegacy;

        if (!isPositive(usdtToVes)) {
            return null;
        }
...
export function useFxCalculatorRates(debouncedAmount: number = 0) {
    const externalRatesApiUrl = (import.meta.env.VITE_RATES_API_URL as string | undefined)?.trim();
    const { rates } = useExchangeRates();
    const { fetchP2PRate } = useBinanceAPI();
    ...
    const refresh = useCallback(async () => {
        ...
        try {
            const [
                ...
                fetchUsdtFiatVps('VES', debouncedAmount, fetchP2PRate),
                externalRatesApiUrl ? fetchUsdtVesFromExternalRatesApi(externalRatesApiUrl) : Promise.resolve(null),
            ]);

            const usdtVes = usdtVesVps || usdtVesExternal || usdtVesCryptoYa;
    ...
    }, [externalRatesApiUrl, rates.VES, rates.COP, rates.PEN, rates.purchaseRateVES]);
