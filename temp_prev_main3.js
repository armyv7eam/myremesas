...
// UIDs de administradores. Se pueden a+地dir m+疄 separados por comas.
...
const initialAuthToken = null; // No estamos usando este m+差odo por ahora.
...
// Configuraci+好 de m+甏genes (se puede sobrescribir desde Firestore)
...
// --- DECLARACI+攓 DE VARIABLES DEL DOM (INICIALIZACI+攓 MOVIDA A initializeDOM) ---
...
// Nuevas variables para autenticaci+好 por email
...
function initializeDOM() {
    ...
    // Elementos para autenticaci+好 por email
...
async function initializeFirebase() {
    ...
    try {
        ...
        if (!firebaseConfig) {
            ...
            authStatus.textContent = "Error: Configuraci+好 de Firebase no disponible.";
        ...
        onAuthStateChanged(auth, async (user) => {
            ...
            if (user && !user.isAnonymous) {
                ...
                // Usuario con email y contrase+地
                ...
                // Si el usuario no es an+好imo, muestra su email. Si no, muestra su UID.
                ...
                if (user.email) {
                    ...
                    logoutButton.classList.remove('hidden'); // Muestra bot+好 de logout

                } else {

                    userIdDisplay.textContent = `An+好imo (${userId.substring(0, 8)}...)`;

                    if (authFormsSection) authFormsSection.classList.remove('hidden'); // Muestra formularios

                    logoutButton.classList.add('hidden'); // Oculta bot+好 de logout
                ...
                // No hay usuario logueado o es an+好imo. Mostrar formularios de registro/login.

                if (user && user.isAnonymous) {

                    await signOut(auth); // Cerramos sesi+好 an+好ima para forzar login/registro
                ...
                if(authStatus) authStatus.textContent = "Por favor, inicie sesi+好 o reg+︿trese.";
                ...
                if (historyContainer) historyContainer.innerHTML = '<p class="text-gray-500 text-sm p-2">Inicie sesi+好 para ver su historial.</p>';
 ...
 * **NUEVA FUNCI+攓**: Copia texto al portapapeles y muestra feedback.
...
/**
 * Genera el bloque de HTML para los detalles de una cuenta, con un interlineado m+疄 ajustado
 * y un +好ico bot+好 para copiar todos los datos.
 */
function buildAccountDetailsMarkup(account) {
    const details = [
        ...
        { label: 'N+妃ero', value: account.accountNumber },
    ];
